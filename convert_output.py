...
class TxtFormatter:
    ...
    @classmethod
    def format_chunk(cls, chunk, index) -> str:
        _ = index  # Acknowledge parameter for consistency
        text = chunk["text"]
        return f"{text}\n"
...
def convert(input_path, output_format, output_dir, verbose) -> None:
    ...
    with open(
        os.path.join(output_dir, f"output.{output_format}"), "w", encoding="utf-8"
    ) as file:
        file.write(string)
...
def main() -> None:
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Print each VTT entry as it's added"
    )
