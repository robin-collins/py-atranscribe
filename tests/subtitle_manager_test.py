...
EXPECTED_SEGMENT_END_TIME = 5.0
EXPECTED_SEGMENTS_COUNT = 2
EXPECTED_FORMATS_COUNT = 3
EXPECTED_SUMMARY_SEGMENTS = 3
EXPECTED_SUMMARY_DURATION = 6.0
EXPECTED_WORD_COUNT = 13
EXPECTED_NUM_SPEAKERS = 2
EXPECTED_AVERAGE_SEGMENT_LENGTH = 2.0
EXPECTED_SPEECH_RATE = 130.0
EXPECTED_MERGED_SEGMENTS = 2
EXPECTED_MERGED_END_TIME = 2.0
EXPECTED_MERGED_START_TIME = 2.0
EXPECTED_MERGED_FINAL_END_TIME = 4.0
EXPECTED_FILTERED_SEGMENTS = 2
...
class TestTranscriptSegment:
    ...
    def test_create_segment(self) -> None:
        ...
        assert segment.end == EXPECTED_SEGMENT_END_TIME
...
class TestSubtitleManager:
    ...
    def test_convert_segments_filters_empty_text(self) -> None:
        ...
        assert len(segments) == EXPECTED_SEGMENTS_COUNT  # Empty segments filtered out

    def test_format_text_with_speaker(self) -> None:
    ...
    def test_seconds_to_webvtt_time(self) -> None:
...
class TestSubtitleFormats:
    ...
    def test_save_webvtt(self) -> None:
    ...
    def test_save_json(self) -> None:
        ...
        try:
            ...
            assert len(data["segments"]) == EXPECTED_SEGMENTS_COUNT
            assert data["metadata"]["test_key"] == "test_value"
            assert data["statistics"]["total_segments"] == EXPECTED_SEGMENTS_COUNT

        finally:
            output_path.unlink()

    def test_save_tsv(self) -> None:
...
class TestMultipleFormats:
    ...
    def test_save_transcripts_multiple_formats(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            assert len(result) == EXPECTED_FORMATS_COUNT
    ...
    def test_save_transcripts_empty_segments(self) -> None:
    ...
    def test_save_transcripts_unsupported_format(self) -> None:
...
class TestTranscriptSummary:
    ...
    def test_get_transcript_summary(self) -> None:
        """Test transcript summary generation."""
        segments = [
            {
                "start": 0.0,
        ...
        assert summary["total_segments"] == EXPECTED_SUMMARY_SEGMENTS
        assert summary["total_duration"] == EXPECTED_SUMMARY_DURATION
        assert (
            summary["word_count"] == EXPECTED_WORD_COUNT
        )  # Total words across all segments
        assert summary["num_speakers"] == EXPECTED_NUM_SPEAKERS
        assert "SPEAKER_00" in summary["speakers"]
        assert "SPEAKER_01" in summary["speakers"]
        assert summary["average_segment_length"] == EXPECTED_AVERAGE_SEGMENT_LENGTH
        assert (
            summary["speech_rate"] == EXPECTED_SPEECH_RATE
        )  # 13 words / (6 seconds / 60)

    def test_get_transcript_summary_empty(self) -> None:
...
class TestSegmentProcessing:
    ...
    def test_merge_short_segments(self) -> None:
        ...
        assert len(merged) == EXPECTED_MERGED_SEGMENTS
        assert merged[0]["text"] == "Hi How are you?"
        assert merged[0]["start"] == 0.0
        assert merged[0]["end"] == EXPECTED_MERGED_END_TIME
        assert merged[1]["text"] == "Good Thanks for asking"
        assert merged[1]["start"] == EXPECTED_MERGED_START_TIME
        assert merged[1]["end"] == EXPECTED_MERGED_FINAL_END_TIME

    def test_merge_short_segments_different_speakers(self) -> None:
    ...
    def test_filter_low_confidence_segments(self) -> None:
        ...
        filtered = self.manager.filter_low_confidence_segments(
            segments,
            min_confidence=-1.5,
        )

        # Should keep only segments above threshold
        assert len(filtered) == EXPECTED_FILTERED_SEGMENTS
